...
def optimise_by_annealing(f, n,
    ...
    for i in range(max_iter):
        ...
        if candidate_loss < loss:
            ...
            if np.random.uniform() < np.exp(-(candidate_loss - current_loss) / temperature):
                xs = candidate
        temperature *= cool
    return xs, current_loss

def cumulative_normal(x, mu=0, sigma=1.0):
    return 0.5 * (1 + sp.special.erf((x - mu)/(np.sqrt(2) * sigma)))
...
optimise_by_annealing(f, 2, scale=(lambda tabs: D * tabs / sum(tabs)),
                      tol=1.0)
